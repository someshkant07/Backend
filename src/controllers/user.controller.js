...
const updateUserAvatar = asyncHandler(async (req, res) => {
  ...
  if (!avatar.url) {
    throw new ApiError(
      400,
      "error while uploading on avatar, avatar url is  missing"
    );
  }
...
const updateUserCoverImage = asyncHandler(async (req, res) => {
  ...
  if (!coverImage.url) {
    throw new ApiError(
      400,
      "error while uploading on cover Iamge, cover image url is  missing"
    );
  }
  ...
  return res
    .status(200)
    .json(new ApiResponse(200, user, "cover image uploaded successfull"));
});
